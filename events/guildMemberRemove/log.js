...
module.exports = async ({ databases: { guilds }, params: [member, preview] }) => {
  ...
  const [guild = {}] = await guilds.query(
    ...
    LEFT JOIN guilds.logs_extra AS ex ON ex.guild = "${member.guild.id}" AND ex.type = "leave" 
    ...
    WHERE logs.guild = "${member.guild.id}" AND logs.type = "leave";`
  )
  ...
  if (channel) {
    if (guild.display == 'canvas' && channel.permissionsFor(member.guild.me).has(1n << 15n)) {
      ...
      /* Icon */
      ctx.drawImage(await icons.fetch('others/sign-out'), 42, 232, 48, 48)
      ...
      channel.send({ files: [{ attachment: ctx.canvas.toBuffer(), name: 'leave.png' }] })
    } else channel.send({ embeds: [{ color, author: { name: `${member.user.tag} a quitté la guilde`, icon_url: member.user.displayAvatarURL({ dynamic: true }) } }] })
  }
}
